    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div className="card w-full max-w-lg max-h-[80vh] flex flex-col animate-slide-up" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
          {comments.length === 0 && (
            <p className="text-gray-500 text-center py-8 text-sm">İlk yorumu sen yap!</p>
          )}
          {comments.map(c => (
            <div key={c.id} className="flex gap-3">
              ...
              <div>
                <p className="text-sm"><span className="font-semibold text-brand-400">@{c.profiles?.username}</span> <span className="text-gray-300">{c.text}</span></p>
